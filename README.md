...
st.title("ðŸ§  ULTIMATE QUANT TERMINAL (HEDGE-FUND STYLE)")
...
if len(price_buffer) > 50:
    ...
    if vol < 0.05:
        regime = "ðŸ§Š CONTROLLED"
    elif vol > 0.4:
        regime = "ðŸ”¥ EXTREME VOL"
    else:
        regime = "âš–ï¸ NORMAL"
    ...
    # âœ… FIXED: Proper indentation for decision logic
    if score >= 3:
        decision = "ðŸš¨ BLOCK"
    elif confidence > 0.7 and trend_strength > 0.3:
        decision = "ðŸŽ¯ EXECUTE"
    else:
        decision = "âš ï¸ HOLD"
    ...
    st.subheader("ðŸ§  Regime")
    st.write(regime)

    st.subheader("ðŸŽ¯ Decision Engine")
    st.write(decision)

    st.subheader("ðŸ“ˆ Equity Curve")
    st.line_chart(equity)

    st.subheader("ðŸ“Š Price Action")
    st.line_chart(df["price"])

    st.subheader("ðŸš¨ System Alerts")

    if decision == "ðŸš¨ BLOCK":
        st.error("MARKET MANIPULATION DETECTED")
    elif decision == "âš ï¸ HOLD":
